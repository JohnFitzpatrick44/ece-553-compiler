functor Semant(Tr: TRANSLATE) :> Semantic
where type frag = Tr.frag = 
struct
	type frag = Tr.frag
    ...
    let
      fun simpleVar(sym, pos) =
        case S.look(venv, sym) of
          SOME(E.VarEntry{access, ty, isLoopVar}) =>
    ...
    let
        ...
        Log.flatMap(checkMatch(leftTy, rightTy, pos), fn () => 
          case leftTy of 
            Types.STRING => (
              ...
              | A.NeqOp => Log.success({exp=Tr.stringNEQ(leftExp, rightExp), ty=Types.INT})
            )
      ...
      and forExp(var, escape, lo, hi, body, pos) =
        let
          ...
          val venvNew = S.enter (venv, var, E.VarEntry {access=idxAccess, ty=Types.INT, isLoopVar=true})
        in
    ...
    let
        ...
        in
          ...
          Log.flatMap(typeToString(pos)(decType), fn decTypeStr =>
            ...
            then 
                ...
                Log.success({venv=S.enter(venv, name, E.VarEntry{access=access,ty=decType,isLoopVar=false}),
                              tenv=tenv,
                              exp=SOME(Tr.assignExp(Tr.simpleVar(access, lvl), initExp))})
            else Log.failure(
                  {venv=S.enter(venv, name, E.VarEntry{access=access,ty=decType,isLoopVar=false}),
                   tenv=tenv, 
                   exp=NONE}, pos,
      ...
      fun typeDecs(typedecs) =
        let
          ...
          val logs = 
            Log.flatMap(headerE, fn headerE => 
            Log.flatMap(uniqueSet, fn uniqueSet =>
              Log.all (
                map (fn ({name, ty, pos}) =>
                       Log.map(transTy(headerE, ty), fn ty =>
                         let val SOME(Types.NAME(_, r)) = S.look(headerE, name)
                         in r := SOME(ty)
                         end))
                    uniqueSet
            )))

          fun checkCycle(headerE, uniqueSet) = foldr
            (fn ({name, ty, pos}, acc) => 
              Log.flatMap(acc, fn acc =>
                case S.look(headerE, name) of
                  SOME(t) => 
        ...
        in
          Log.flatMap(uniqueSet, fn uniqueSet => 
          Log.flatMap(headerE, fn headerE => 
          Log.flatMap(checkCycle(headerE, uniqueSet), fn () => 
          Log.map(logs, fn (_) => { venv = venv, tenv = headerE, exp=NONE }))))
        end
      ...
      fun functionDecs(fundecs) =
        let
              ...
              (fn ({name, params, result, body, pos}, acc) =>
                ...
                in
                  ...
                  Log.map(resultToTy result, fn resultTy =>
                    S.enter(env, name, 
                            E.FunEntry{level=lvl, label=funcLabel, formals=paramTys, result=resultTy}))))
                end)
              (Log.success venv)
              uniqueSet
            )
          ...
          fun withParams(name, params, venv) = 
            let
              val SOME(E.FunEntry{level, label, formals, result}) = S.look(venv, name)
              val fncLevel = Tr.newLevel{parent=level, name=label, formals=extractEsc params}
              val venv' = foldl
                (fn ((field, access), acc) =>
                  Log.flatMap(acc, fn acc =>
                  Log.map(fieldToTy field, fn fieldTy =>
                     S.enter(acc, fieldName field, E.VarEntry{access=access, ty=fieldTy, isLoopVar=false}))))
                (Log.success venv)
          ...
          fun bodyCheck({name, params, result, body, pos}) =
            Log.flatMap(headerE, fn env =>
            Log.flatMap(withParams(name, params, env), fn (funLevel, funE) =>
            Log.flatMap(transExp(funLevel, funE, tenv, body, loops), fn ({exp=bodyExp, ty=resTy}) =>
        ...
        in
          ...
          Log.map(headerE, fn headerE =>
            { venv = headerE, tenv = tenv, exp=NONE }))
        end
    in
      case declaration of
